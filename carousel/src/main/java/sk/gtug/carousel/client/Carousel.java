...
public class Carousel extends LayoutPanel {
    ...
    private void moveRectangle(int delta) {
        ...
        for (CarouselImagePanel panel : panels) {
            ...
            AnimationInfo animInfo = panel.getAnimationInfo(rectForPhase, steps, lastPhase);
    ...
    private void animate(final Map<CarouselImagePanel, CarouselImagePanel.AnimationInfo> animInfo, final int steps, int delta) {
        final Set<CarouselImagePanel> panels = animInfo.keySet();
        this.active = true;
        for (CarouselImagePanel panel : panels) {
            if (animInfo.get(panel).lastPhase == 3)
                panel.setImageHandle(delta > 0 ? getImageUrl(actualImageIndex + 2) : getImageUrl(actualImageIndex - 2));
        }
        new Timer() {
            int duration = 150;
            double step = 1;
            ...
            public void run() {
                if (step == steps) {
                    for (CarouselImagePanel panel : panels) {
                        ...
                        panel.updatePanel(panel.calculateStepRect(step, aniInfo.stepX, aniInfo.stepY, aniInfo.stepWidth, aniInfo.lastRect));
                    }
                    ...
                    for (CarouselImagePanel panel : panels) {
                        ...
                        panel.updatePanel(panel.calculateStepRect(myStep, aniInfo.stepX, aniInfo.stepY, aniInfo.stepWidth, aniInfo.lastRect));
