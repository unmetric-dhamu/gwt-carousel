...
public class Carousel extends LayoutPanel {
    ...
    private void animate(final Map<CarouselImagePanel, CarouselImagePanel.AnimationInfo> animInfo, final int steps) {
    ...
    private void updateImageInPanels() {
        for (CarouselImagePanel panel : panels) {
            ...
            if (panel.getPhase() == 1)
                panel.setImageHandle(imageProvider.getImageUrl(actualImageIndex - 1));
            if (panel.getPhase() == 2)
                panel.setImageHandle(imageProvider.getImageUrl(actualImageIndex - 2));
            if (panel.getPhase() == 3)
